...
def parse_args():
    ...
    parser.add_argument('--impute', type=str, default=-1)
...
def run(args):
    ...
    pipeline = sklearn.pipeline.Pipeline(steps=[('imputer', sklearn.preprocessing.Imputer(strategy='median')),
                                                ('classifier', sklearn.ensemble.RandomForestRegressor(n_estimators=16))])

    meta = algsel.utils.ModelWrapper(pipeline, True)
    ...
    print(model_score, gap_score_single, gaps_stdev_single)
